...
export function VoiceControls({
  ...
  return (
    <div className="mt-3 sm:mt-4 xl:mt-5 flex flex-col gap-2.5 sm:gap-3">
      {/* Start / Stop button */}
      <div className="flex justify-center">
        <button
          onClick={onToggle}
          disabled={isTogglingConversation}
          className="inline-flex min-w-[110px] sm:min-w-[130px] justify-center items-center gap-2 rounded-full border border-cyan-300/50 bg-cyan-400/15 px-3 sm:px-4 py-2 text-sm sm:text-base font-bold tracking-[0.12em] text-cyan-100 transition hover:bg-cyan-300/20 disabled:cursor-not-allowed disabled:opacity-70"
        >
      ...
      <div className="flex mt-2 sm:mt-3 justify-center">
        <div className="inline-flex overflow-hidden rounded-full border border-slate-700/80 bg-slate-900/70 p-0.5 sm:p-1 text-[11px] sm:text-xs tracking-[0.11em]">
          <button
            onClick={() => onSelectSpeaker("monowar")}
            className={`rounded-full px-2.5 sm:px-3 py-1.5 sm:py-2 transition ${
      ...
      <div className="flex justify-end">
        <button
          ...
          className={`inline-flex items-center gap-2 rounded-full border px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold tracking-[0.12em] transition disabled:cursor-not-allowed disabled:opacity-50 ${
            hasPendingInstructionChange
